...
```bash
git clone https://github.com/devSviat/Checkbox-OkayCMS.git
```
...
```
{OkayCMS_root}/
├── Okay/Modules/Sviat/Checkbox/     ← модуль
└── tests/Modules/Sviat/Checkbox/     ← його тести
```
...
```bash
cd {OkayCMS_root}
mkdir -p tests/Modules/Sviat/Checkbox
cp -r ~/Checkbox-OkayCMS/tests/. tests/Modules/Sviat/Checkbox/
```
...
```bash
cd {OkayCMS_root}
vendor/bin/phpunit tests/Modules/Sviat/Checkbox
```
...
2. Неймспейс — `Modules\Sviat\Checkbox`.
...
use Okay\Modules\Sviat\Checkbox\Services\SomeService;
